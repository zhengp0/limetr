"""
Test Linear Mixed Effects Module
"""
import pytest
import numpy as np
...
from limetr.models.mixed_effects import MeModel
...
def model(data, fevar, revar, inlier_pct):
    return MeModel(data, fevar, revar, inlier_pct=inlier_pct)
...
def test_validate_fevar(data, my_fevar, revar, inlier_pct):
    with pytest.raises(ValueError):
        MeModel(data, my_fevar, revar, inlier_pct=inlier_pct)
...
def test_get_obs_varmat(model):
    my_varmat = np.diag(model.data.obs_se**2)
    assert np.allclose(model.get_obs_varmat(), my_varmat)


@pytest.mark.parametrize("gamma", [np.zeros(2)])
def test_get_varmat(model, gamma):
    my_varmat = model.get_varmat(gamma)
    assert np.allclose(my_varmat, model.get_obs_varmat())
...
def test_objective(model, var):
    ...
    r = model.get_residual(beta)
    d = np.diag(model.get_obs_varmat())
    tr_obj = 0.5*(np.sum(r**2/d) + np.sum(np.log(d)))
...
def test_hessian(model, var):
    ...
    tr_hess = ad_jacobian(model.gradient, var, (var.size, var.size))
...
def test_get_random_effects_nonzero_gamma(model, var):
    ...
    v = split_by_sizes(np.diag(model.get_obs_varmat()), model.data.group_sizes)
