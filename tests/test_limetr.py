...
def lmtr_test_problem_lasso():
    ...
    return LimeTr(n, k_beta, k_gamma, Y, F, JF, Z, S=S, uprior=uprior, lprior=lprior)
...
def lmtr_objective(lmtr, x):
    ...
    D = block_diag(
        *[np.diag(v) + (z * gamma).dot(z.T) for v, z in zip(v_study, z_study)]
    )
...
def test_limetr_objective():
    ...
    # -------------------------------------------------------------------------
    model = lmtr_test_problem(
        use_constraints=True, use_regularizer=True, use_uprior=True, use_gprior=True
    )
...
def test_estimate_re():
    ...
    g = np.vstack(
        [(zi.T / vi).dot(ri) + ui / model.gamma for zi, vi, ri, ui in zip(z, v, r, re)]
    )
...
def test_performance():
    ...
    conditional_r2 = get_conditional_R2(model)
    marginal_r2 = get_marginal_R2(model)
    rmse = get_rmse(model)
